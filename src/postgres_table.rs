...
use datafusion::arrow::array::{
    ...
    StringArray, TimestampNanosecondArray, Date32Array, GenericBinaryArray, ArrayBuilder,
};
...
impl TableProvider for PostgresTable {
    ...
    async fn scan(
        ...
        let selected_field_names: Vec<String> = projected_schema.fields().iter().map(|f| f.name().clone()).collect();
        let sql_select_cols = if selected_field_names.is_empty() || selected_field_names.len() == self.schema.fields().len() {
        ...
        let rows = self
            ...
            .query(&query, &[])
            .await
            .map_err(|pg_err| DataFusionError::External(Box::new(IglooError::Postgres(pg_err))))?;

        if rows.is_empty() {
            let batch = RecordBatch::new_empty(projected_schema.clone());
            return Ok(Arc::new(MemoryExec::try_new(&[vec![batch]], self.schema(), projection.cloned())?));
        }
        ...
        for (col_idx, field) in projected_schema.fields().iter().enumerate() {
                ...
                ($builder:expr, $pg_type:ty, $arrow_builder_type:ty) => {
                    {
                        ...
                        for row in &rows {
                            match row.try_get::<$usize, Option<$pg_type>>(col_idx) {
                                ...
                                Err(e) => return Err(DataFusionError::External(Box::new(IglooError::Postgres(e))))
                            }
                        }
                ...
                ($builder:expr, chrono::NaiveDateTime, $arrow_builder_type:ty) => {
                    {
                        ...
                        for row in &rows {
                            match row.try_get::<$usize, Option<chrono::NaiveDateTime>>(col_idx) {
                                Ok(Some(val)) => builder.append_value(val.timestamp_nanos_opt().unwrap_or_default()),
                                Ok(None) => builder.append_null(),
                                Err(e) => return Err(DataFusionError::External(Box::new(IglooError::Postgres(e))))
                            }
                        }
                ...
                ($builder:expr, chrono::NaiveDate, $arrow_builder_type:ty) => {
                    {
                        ...
                        for row in &rows {
                            match row.try_get::<$usize, Option<chrono::NaiveDate>>(col_idx) {
                                Ok(Some(val)) => builder.append_value(val.signed_duration_since(epoch).num_days() as i32),
                                Ok(None) => builder.append_null(),
                                Err(e) => return Err(DataFusionError::External(Box::new(IglooError::Postgres(e))))
                            }
                        }
            ...
            let array_ref: ArrayRef = match field.data_type() {
                DataType::Int16 => append_col_data!(Int16Array::builder(rows.len()), i16, Int16Array),
                DataType::Int32 => append_col_data!(Int32Array::builder(rows.len()), i32, Int32Array),
                DataType::Int64 => append_col_data!(Int64Array::builder(rows.len()), i64, Int64Array),
                DataType::Float32 => append_col_data!(Float32Array::builder(rows.len()), f32, Float32Array),
                DataType::Float64 => append_col_data!(Float64Array::builder(rows.len()), f64, Float64Array),
                DataType::Utf8 => append_col_data!(StringArray::builder(rows.len()), String, StringArray),
                DataType::Boolean => append_col_data!(BooleanArray::builder(rows.len()), bool, BooleanArray),
                DataType::Timestamp(TimeUnit::Nanosecond, None) => { // Assuming UTC if no timezone specified in Arrow schema
                    append_col_data!(TimestampNanosecondArray::builder(rows.len()), chrono::NaiveDateTime, TimestampNanosecondArray)
                }
                DataType::Date32 => append_col_data!(Date32Array::builder(rows.len()), chrono::NaiveDate, Date32Array),
                DataType::Binary => append_col_data!(GenericBinaryArray::<i32>::builder(rows.len()), Vec<u8>, GenericBinaryArray<i32>),
                dt => {
        ...
        Ok(Arc::new(MemoryExec::try_new(&[vec![batch]], self.schema(), projection.cloned())?))
    }
}
